...
// === Servidor 1: Compilador (Puerto 3000) ===
...
// === Servidor 2: Archivos estáticos (Puerto 3001) ===
...
// === Servidor 3: Editor colaborativo (Puerto 3002) ===
const app = express();
...
let usuariosConectados = new Map(); // socketId -> {username, color, pestañaActiva}
...
io.on('connection', (socket) => {
  ...
  socket.on('crear-pestaña', (data) => {
    ...
    const nombreTab = nombre || `Pestaña ${contadorTabs}`;
    ...
    console.log(`Nueva pestaña creada: ${nombreTab} (${tabId}) por usuario ${socket.id}`);
  });
...
server.listen(PORT, HOST, () => {
  ...
  console.log(`   🌐 Red:      http://${ipLocal}:3000`);
  console.log(`\n📂 Servidor de Archivos Estáticos:`);
  console.log(`   📍 Local:    http://localhost:3001`);
  console.log(`   🌐 Red:      http://${ipLocal}:3001`);
